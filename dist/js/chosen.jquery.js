  ...
  Chosen = class Chosen extends AbstractChosen {
    ...
    register_observers() {
      ...
      if (this.is_multiple) {
        ...
        return this.selected_item.on('keydown.chosen', (evt) => {
          var ref;
          if (((ref = evt.which) === 13 || ref === 32) && !this.is_disabled) {
            evt.preventDefault();
            this.results_toggle();
          }
        });
    ...
    set_aria_labels() {
      ...
      if (this.form_field.attributes["aria-labelledby"]) {
        this.search_field.attr("aria-labelledby", this.form_field_jq.attr("aria-labelledby"));
        ...
        this.search_field.attr("aria-labelledby", labelledbyList);
      }
      if (this.form_field.attributes["aria-describedby"]) {
        return this.search_field.attr("aria-describedby", this.form_field_jq.attr("aria-describedby"));
      }
    }
    ...
    search_field_disabled() {
      ...
      if (!this.is_multiple) {
        ...
        this.selected_item.attr('aria-disabled', this.is_disabled);
        this.selected_item.attr('tabindex', this.is_disabled ? -1 : 0);
      }
      if (this.is_disabled) {
    ...
    results_show() {
      ...
      if (!this.is_multiple) {
        this.selected_item.attr("aria-expanded", true);
      }
    ...
    results_hide() {
      if (this.results_showing) {
        ...
        this.container.removeClass("chosen-dropup");
        if (!this.is_multiple) {
          this.selected_item.attr("aria-expanded", false);
        }
