...
<span class="chosen-results-status visually-hidden" role="status" aria-live="polite" aria-atomic="true"></span>`;
  ...
  Chosen = class Chosen extends AbstractChosen {
    ...
    set_up_html() {
      ...
      this.form_field_jq.css('position', 'absolute').css('opacity', 0);
      if (!this.form_field.required) {
        this.form_field_jq.css('display', 'none');
      }
      this.form_field_jq.after(this.container);
      this.dropdown = this.container.find('div.chosen-drop').first();
      if (this.options.dropdown_width != null) {
        this.dropdown.css("width", this.options.dropdown_width);
      }
      this.search_field = this.container.find('input').first();
      this.search_results = this.container.find('ul.chosen-results').first();
      this.results_status = this.container.find('.chosen-results-status').first();
      this.search_results.attr('id', `${this.result_id_base}-search-results`);
    ...
    register_observers() {
      ...
      this.form_field_jq.on("chosen:updated.chosen", this.form_field_observers.updated);
      ...
      this.search_field.on('input.chosen', (evt) => {
        this.search_if_value_changed(evt);
      });
      this.search_field.on('compositionstart.chosen', (evt) => {
        this.composition_start(evt);
      });
      this.search_field.on('compositionend.chosen', (evt) => {
        this.composition_end(evt);
      });
      ...
      if (this.is_multiple) {
        ...
        return this.selected_item.on('keydown.chosen', (evt) => {
          this.selected_item_keydown(evt);
        });
      }
    }
    ...
    destroy() {
      this.cancel_pending_search();
      $(window).off('pageshow.chosen', this.pageshow_handler);
      $(window).off('blur.chosen', this.window_blur_handler);
      ...
      if (this.form_field_label.length > 0) {
        this.form_field_label.off('mousedown.chosen', this.label_mousedown_handler);
        this.form_field_label.off('click.chosen', this.label_click_handler);
      }
      this.form_field_jq.off("chosen:updated.chosen", this.form_field_observers.updated);
    ...
    set_aria_labels() {
      ...
      for (j = 0, len = ref.length; j < len; j++) {
        name = ref[j];
        this.search_field.removeAttr(name);
        if (!this.is_multiple) {
          this.selected_item.removeAttr(name);
        }
      }
      ...
      for (k = 0, len1 = ref1.length; k < len1; k++) {
        attribute = ref1[k];
        this.search_field.attr(attribute.name, attribute.value);
        if (!this.is_multiple) {
          this.selected_item.attr(attribute.name, attribute.value);
        }
        this.copied_aria_attributes.push(attribute.name);
      }
      this.search_field.attr("aria-owns", this.search_results.attr("id"));
      this.search_field.attr("aria-controls", this.search_results.attr("id"));
      if (!this.is_multiple) {
        this.selected_item.attr("aria-controls", this.search_results.attr("id"));
      }
      if (!this.form_field.attributes["aria-labelledby"] && !this.form_field.attributes["aria-label"] && ((ref2 = this.form_field.labels) != null ? ref2.length : void 0)) {
        ...
        if (!this.is_multiple) {
          this.selected_item.attr("aria-labelledby", labelledbyList);
        }
    ...
    search_field_disabled() {
      ...
      this.is_disabled = this.form_field.disabled || this.form_field_jq.parents('fieldset').is(':disabled') || this.is_read_only;
      this.container.toggleClass('chosen-disabled', this.is_disabled);
      this.container.toggleClass('chosen-readonly', this.is_read_only);
      this.search_field[0].disabled = this.is_disabled;
      if (!this.is_multiple) {
        this.selected_item.attr('aria-disabled', this.is_disabled);
        this.selected_item.attr('tabindex', this.is_disabled ? -1 : this.selected_item_tab_index);
      }
      if (this.is_disabled) {
        return this.close_field();
      }
    ...
    test_active_click(evt) {
      var active_container, active_form_field, active_label;
      active_container = $(evt.target).closest('.chosen-container');
      active_label = this.form_field_label.length && (this.form_field_label.is(evt.target) || this.form_field_label.has(evt.target).length);
      active_form_field = evt.target === this.form_field;
      if (active_label || active_form_field || (this.mousedown_checker(evt) === 'left' && active_container.length && this.container[0] === active_container[0])) {
    ...
    result_clear_highlight() {
      ...
      return this.search_field.removeAttr("aria-activedescendant");
    }
    ...
    results_show() {
      ...
      if (!this.is_multiple) {
        this.selected_item.attr("aria-hidden", true);
      }
    ...
    update_empty_results_state() {
      return this.container.toggleClass('chosen-empty-results', this.is_multiple && !this.create_option && this.search_results.children().length === 0);
    }
    ...
    results_hide() {
      ...
      if (this.results_showing) {
        ...
        if (!this.is_multiple) {
          this.selected_item.removeAttr("aria-hidden");
        }
        if (!(this.is_multiple || this.is_disabled)) {
          this.selected_item.attr("tabindex", this.selected_item_tab_index);
        }
    ...
    set_tab_index(el) {
      ...
      if (this.is_multiple) {
        ...
        this.selected_item.attr("tabindex", this.is_disabled ? -1 : this.selected_item_tab_index);
        this.search_field[0].tabIndex = -1;
      }
      if (this.form_field.required) {
    ...
    set_label_behavior() {
      ...
      if (this.form_field_label.length > 0) {
        this.form_field_label.on('mousedown.chosen', this.label_mousedown_handler);
    ...
    search_results_mouseout(evt) {
      if ($(evt.target).hasClass("active-result") || $(evt.target).parents('.active-result').first().length) {
      ...
      if ($(evt.target).hasClass("group-result")) {
        ...
        $(evt.target).nextAll().each((_, option) => {
          ...
          if (!$(option).hasClass("group-result")) {
            ...
            if ($(option).hasClass('group-option') && $(option).hasClass('active-result') && !((ref = this.results_data[array_index]) != null ? ref.selected : void 0)) {
    ...
    result_for_array_index(array_index) {
      return this.search_results.find(`[data-option-array-index='${array_index}']`).first();
    }
    ...
    winnow_results_after_select(item) {
      var active_results, next_result, scroll_position;
      scroll_position = this.search_results.scrollTop();
      this.winnow_results({
        skip_highlight: true
      });
      active_results = this.search_results.find(".active-result");
      next_result = active_results.filter(function(index, result) {
        return parseInt(result.getAttribute("data-option-array-index"), 10) > item.data["data-option-array-index"];
      }).first();
    ...
    keyhome() {
      ...
      first = this.search_results.find("li.active-result").first();
      if (first.length) {
    ...
    keyend() {
      ...
      last = this.search_results.find("li.active-result").last();
      if (last.length) {
    ...
    keypage(direction) {
      ...
      results = this.search_results.find("li.active-result");
      current_index = results.index(this.result_highlight);
      item_height = this.result_highlight.outerHeight();
      page_size = item_height > 0 ? Math.max(1, Math.floor(this.search_results.innerHeight() / item_height)) : 1;
      target_index = Math.max(0, Math.min(results.length - 1, current_index + direction * page_size));
      return this.result_do_highlight(results.eq(target_index));
    }
