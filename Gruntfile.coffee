...
"""
  ...
  grunt.registerTask 'fix-sourcemaps', 'Fix sourcemap references to be relative', ->
    # Fix build sourcemap URL reference
    css = fs.readFileSync('build/chosen.css', 'utf8')
    ...
    # Fix build sourcemap file paths to be relative
    mapContent = fs.readFileSync('build/chosen.css.map', 'utf8')
    ...
    fs.writeFileSync('build/chosen.css.map', JSON.stringify(mapObj))
