...
"""
    ...
    concat:
      options:
        banner: '<%= comments %>'
      jquery:
        src: ['dist/chosen.jquery.js']
        dest: 'dist/chosen.jquery.js'
      proto:
        src: ['dist/chosen.proto.js']
        dest: 'dist/chosen.proto.js'
      css:
        src: ['dist/chosen.css']
        dest: 'dist/chosen.css'
    ...
    copy:
      ...
      docs:
        files: [
          {expand: true, cwd: 'dist/', src: ['*.js', '*.css', '*.map', 'LICENSE.md'], dest: 'docs/'}
        ]
    ...
    coffee:
      ...
      jquery:
        files:
          'dist/chosen.jquery.js': ['coffee/lib/select-parser.coffee', 'coffee/lib/abstract-chosen.coffee', 'coffee/chosen.jquery.coffee']
      proto:
        files:
          'dist/chosen.proto.js': ['coffee/lib/select-parser.coffee', 'coffee/lib/abstract-chosen.coffee', 'coffee/chosen.proto.coffee']
      test:
        files:
      ...
      jquery:
        options:
          mangle:
            reserved: ['jQuery']
        files:
          'dist/chosen.jquery.min.js': ['dist/chosen.jquery.js']
      proto:
        files:
          'dist/chosen.proto.min.js': ['dist/chosen.proto.js']
    ...
    sass:
      ...
      chosen_css:
        files:
          'dist/chosen.css': 'sass/chosen.scss'
  ...
  grunt.registerTask 'postcss', 'Apply autoprefixer to CSS', ->
    css = fs.readFileSync('dist/chosen.css', 'utf8')
    result = postcss([autoprefixer(overrideBrowserslist: ['last 1 version'])]).process(css, from: 'dist/chosen.css')
    fs.writeFileSync('dist/chosen.css', result.css)
  ...
  grunt.registerTask 'fix-sourcemaps', 'Fix sourcemap references to be relative', ->
    ...
    css = fs.readFileSync('dist/chosen.css', 'utf8')
    css = css.replace(/\/\*# sourceMappingURL=dist\/chosen\.css\.map \*\/$/, '/*# sourceMappingURL=chosen.css.map */')
    fs.writeFileSync('dist/chosen.css', css)

    # Fix dist sourcemap file paths to be relative
    mapContent = fs.readFileSync('dist/chosen.css.map', 'utf8')
    ...
    fs.writeFileSync('dist/chosen.css.map', JSON.stringify(mapObj))
  ...
  grunt.registerTask 'build', ['coffee:jquery', 'coffee:proto', 'sass', 'concat', 'uglify', 'postcss', 'cssmin', 'copy:main', 'copy:docs', 'fix-sourcemaps']
