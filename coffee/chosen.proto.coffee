class @Chosen extends AbstractChosen
  ...
  register_observers: ->
    ...
    @search_field.observe "input", (evt) => this.search_if_value_changed(evt)
    @search_field.observe "compositionstart", (evt) => this.composition_start(evt)
    @search_field.observe "compositionend", (evt) => this.composition_end(evt)
  ...
  result_select: (evt) ->
    ...
    if @result_highlight
      ...
      if @is_multiple
        ...
        this.reset_single_select_options()
        result.writeAttribute('aria-selected', 'false') for result in @search_results.select('[role="option"][aria-selected="true"]')

      high.addClassName("result-selected")
      high.writeAttribute('aria-selected', 'true')
  ...
  winnow_results_after_select: (item) ->
    ...
    this.winnow_results(skip_highlight: true)
    active_results = @search_results.select(".active-result")
    next_result = null
    for result in active_results when parseInt(result.readAttribute("data-option-array-index"), 10) > item.data["data-option-array-index"]
      next_result = result
      break
    next_result ?= active_results.last()
    this.result_do_highlight(next_result) if next_result?
