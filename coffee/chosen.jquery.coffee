...
class Chosen extends AbstractChosen
  ...
  keyhome: ->
    ...
    first = @search_results.find("li.active-result").first()
    this.result_do_highlight(first) if first.length

  keyend: ->
    this.results_show() unless @results_showing
    last = @search_results.find("li.active-result").last()
    this.result_do_highlight(last) if last.length

  keypage: (direction) ->
    this.results_show() unless @results_showing
    return unless @result_highlight

    results = @search_results.find("li.active-result")
    current_index = results.index(@result_highlight)
    item_height = @result_highlight.outerHeight()
    page_size = if item_height > 0 then Math.max(1, Math.floor(@search_results.innerHeight() / item_height)) else 1
    target_index = Math.max(0, Math.min(results.length - 1, current_index + direction * page_size))
    this.result_do_highlight(results.eq(target_index))
